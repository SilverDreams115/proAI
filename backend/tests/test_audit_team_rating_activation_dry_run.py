"""R5.5: dry-run CLI auditor must print a coherent summary and write nothing."""
...
from scripts import audit_team_rating_activation_dry_run as cli
...
def _seed(session):
    ...
    slate = ProgolSlateModel(
        label="cli", draw_code="PG-CLI", week_type="weekend",
...
def test_cli_human_output(tmp_path, capsys):
    ...
    rc = cli.main(["--draw-code", "PG-CLI"])
    out = capsys.readouterr().out

    assert rc == 0
    assert "mode: activation_dry_run" in out
    assert "safe_to_activate: False" in out
    assert "would_route:" in out
    assert "activation_blockers:" in out
    assert "feature_flag_off" in out

    after = _counts(_make_session_existing(tmp_path))
    assert before == after
...
def test_cli_json_output_and_no_writes(tmp_path, capsys):
    ...
    assert rc == 0
    assert payload["mode"] == "activation_dry_run"
    assert payload["safe_to_activate"] is False
    assert payload["summary"]["total_matches"] == 2
    assert "feature_flag_off" in payload["activation_blockers"]
    # Probabilities of routed matches sum to ~1.
    for m in payload["matches"]:
        if m["dry_run_probabilities"]:
            assert abs(sum(m["dry_run_probabilities"].values()) - 1.0) < 1e-3

    after = _counts(_make_session_existing(tmp_path))
    assert before == after


def _make_session_existing(tmp_path):
    from app.db import session as db_session
    from app.db.session import configure_session
